...
public final /*@Interned*/ class VarInfo implements Cloneable, Serializable {
  ...
  //Kaituo
  public boolean isParamOrDerivedParam() {
	  boolean result = false;
	  if(isParamwiththis())
		  result = true;
	  else if(var_kind.equals(VarInfo.VarKind.FUNCTION) || var_kind.equals(VarKind.FUNCTION) || var_kind.equals(VarKind.ARRAY) || var_kind.equals(VarInfo.VarKind.ARRAY)) {
		  VarInfo eParam = enclosing_param();
	      if (eParam != null && eParam.isParamwiththis() && !eParam.name().equals("this"))
	        result = true;
	      else if (derived != null) {
	        for (VarInfo vi : derived.getBases()) {
	          VarInfo derivedEParam = vi.enclosing_param();
	          if (derivedEParam != null && derivedEParam.isParamwiththis() && !derivedEParam.name().equals("this")) {
	            result = true;
	            break;
	          }
	        }
  ...
  public boolean isReturn() {
	  ...
	  if(name().equals("return"))
		  ...
		  VarInfo eParam = enclosing_var;
	      if (eParam != null && eParam.name().equals("return"))
	        result = true;
	      else {
	    	  VarInfo eParam2 = enclosing_var.enclosing_var;
	    	  if (eParam2 != null && eParam2.name().equals("return"))
	  	        result = true;
	      }
	  }
  ...
  //kaituo
  /** Returns whether or not this variable is a parameter **/
  public boolean isParamwiththis() {
    if (FileIO.new_decl_format) {
      return var_flags.contains (VarFlags.IS_PARAM) && !name().equals("this");
    } else {
      return aux.isParam(); // VIN
    }
  }
